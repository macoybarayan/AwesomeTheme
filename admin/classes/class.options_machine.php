...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'create_menu':
					$mini ='';
					$menu_url = get_bloginfo('url');
					if(!isset($value['mod'])) $value['mod'] = '';
					if($value['mod'] == 'mini') { $mini = 'mini';}
					$output .= '<a href="'.$menu_url.'/wp-admin/nav-menus.php" class="of-input '.$mini.' button" id="'. $value['id'] .'">Create Menu</a>';
				break;
				...
				case 'text':
					...
					$label = "";
					if($value['label'] != "") :
						$label = '<div class="label">'. $value['label']. '</div>';
					endif;
					...
					$output .= $label.'<input class="of-input '.$mini.'" name="'.$value['id'].'" id="'. $value['id'] .'" type="'. $value['type'] .'" value="'. $t_value .'" />';
				break;
				...
				case 'select':
					...
					$output .= $label.'<div class="select_wrapper ' . $mini . '">';
				...
				case 'multiselect':
					$select_value = isset($data[$value['id']]) && !empty($data[$value['id']]) ? $data[$value['id']] : array();
					
					$output .= '<select id="'.$value['id'].'" multiple="multiple" data-placeholder="'.$value['std'].'" class="select of-input" name="'.$value['id'].'[]" >';
						foreach ($value['options'] as $key => $option) {
							$selected = (is_array($select_value) && in_array($option, $select_value)) ? ' selected="selected"' : '';
							$output .= '<option value="' . $option. '" ' . $selected . '>'. $option .'</option>';
						}
					$output .= '</select>';
					$output .= '<div class="multiselect_wrapper"></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$output .= $label.'<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
				break;
				...
				case "color":
					...
					$output .= $label.'<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $smof_data[$value['id']] . '"' . $default_color .' />';
				...
				case 'typography':
					...
					if(isset($typography_stored['size'])) {
						$output .= $label.'<div class="select_wrapper typography-size" original-title="Font size">';
				...
				case 'border':
					...
					$output .= '<select class="of-border of-border-width select" name="'.$value['id'].'[width]" id="'. $value['id'].'_width">';
						for ($i = 0; $i < 21; $i++){ 
						$output .= '<option value="'. $i .'px" ' . selected($border_stored['width'], $i, false) . '>'. $i .'px</option>';				 }
					$output .= '</select></div>';
				...
				case 'slider':
					...
					$output .= $label.'<div class="slider"><ul id="'.$value['id'].'">';
				...
				case 'featured':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_featured_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button featured_add_button">Add New Featured Content</a></div>';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<div style="margin-bottom:10px;"><strong style="font-size:11px;">'. $value['label'] . '</strong></div><textarea id="export_data" rows="8">'.base64_encode(serialize($smof_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
			...
			if ( $value['type'] != 'heading') { 
				...
				if(!isset($value['desc'])){ $explain_value = ''; } else{ 
					if($value['label'] != "") :
						...
						$explain_value = '<div class="explain '. $has_label .'">'. $value['desc'] .'</div>'."\n"; 
				} 
	...
	public static function optionsframework_featured_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>Featured Content '.$order.'</strong>';
		}
